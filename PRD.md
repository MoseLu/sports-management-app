...
> Product scope for the Vite + React 18 + Capacitor sports management
...
- **Public surface**:
  - Web SPA built by Vite + React 18
...
1. Vite 5 + React 18 + TypeScript 5 SPA scaffold.
2. React Router v6 routing (`createHashRouter` for Capacitor WebView).
3. Ant Design 5 + `@ant-design/icons` as the UI component library.
4. Zustand 4 for state stores.
5. i18next + react-i18next for internationalization.
6. axios for HTTP, with a single shared client under `src/lib/axios.ts`.
7. ESLint flat config + Prettier.
8. Capacitor 7 Android shell under `src-capacitor/`.
9. Root documentation suite aligned with the workspace deep-init-pro standard.
10. Backend placeholder under `backend/` (no real contract yet).
...
- iOS is not in scope; only the Capacitor Android shell is wired.
...
| `PRD-AC-003` | `src/i18n/index.ts` is the only place that calls `i18n.use()` / `i18n.init()`. | `grep -rE "i18n\\.use\\|initReactI18next" src/` returns only `src/i18n/index.ts` |
...
- **US-OPS-1**: As an Axi operator, I can run `pnpm dev` and see the Vite
  dev server come up.
- **US-OPS-2**: As an Axi operator, I can run `pnpm build` and get a
  production SPA bundle in `dist/`.
- **US-CAP-1**: As an Axi operator, I can run `pnpm build && pnpm cap:sync`
  and the Vite output is copied into the Android shell.
- **US-I18N-1** (future): As a Chinese-speaking user, I can switch the UI
  to `zh-CN` once a real locale is added.
